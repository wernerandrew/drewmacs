...
;; SUPER experimental - really doesn't work yet
(defun my-guess-best-python-root-for-buffer (buf repo-sentry init-sentry)
  "Guesses that the python root is the less 'deep' of either:
     -- the root directory of the repository, or
     -- the directory before the first directory after the root
        having an __init__.py file."
  (interactive)
  ;; small helper to get extra list elements
  (defun get-extra-list-elements (base extra)
    (cond ((not base) extra)
          ((string= (car base) (car extra))
           (get-extra-list-elements (cdr base) (cdr extra)))
          ;; should never get here
          (t (list base extra))));; (error "mismatch between elements"))))
  ;; normalize directory list
  (defun normalize-dir-list (dirs)
    (delq nil
          (mapcar (lambda (x) (and (not (string= x "")) x)) dirs)))
  ;; a little something to try to find the "best" root directory
  (defun try-find-best-root (current remaining)
    (cond ((not remaining) nil)
          (t
           (let* ((test-directory (append current (list (car remaining))))
                    (test-file (mapconcat 'identity
                                          (append test-directory (list init-sentry))
                                          "/")))
             (if (file-exists-p test-file)
                 (mapconcat 'identity current "/")
               (try-find-best-root test-directory (cdr remaining)))))))

  (let* ((base-directory (expand-file-name
                          (file-name-directory (buffer-file-name buf))))
         (base-directory-dirs (normalize-dir-list
                               (split-string base-directory "/")))
         (project-root (expand-file-name
                        (vc-find-root base-directory repo-sentry)))
         (project-root-dirs (normalize-dir-list
                             (split-string project-root "/")))
         (extra-dirs (get-extra-list-elements project-root-dirs base-directory-dirs)))
    (try-find-best-root project-root-dirs extra-dirs)))
